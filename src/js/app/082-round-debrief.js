  /* ===================== MATCH DEBRIEF — the gym-to-pitch loop ===================== */
  // The whole app points at the pitch; this is where the pitch reports back.
  // A ~20-second post-match ritual: minutes played (optional), top speed, how you
  // played, and how the BODY held up. Top speed feeds the top-speed trend
  // automatically (a match best triggers the PR moment), and Stats grows an
  // "On the pitch" card — the proof the work is paying off in real matches.
  // (Stored keys stay ff_rounds / .score / .drive — never renamed.)
  ...
  var RD_DRIVING={ bomb:"On fire", norm:"Solid", short:"Quiet" };
  var RD_ENERGY={ strong:"Strong all 90", faded:"Faded late", gassed:"Gassed" };
  var rdSel={ driving:null, energy:null };

  function rdEnsure(){
    if($("rdModal")) return;
    var m=document.createElement("div"); m.id="rdModal"; m.className="swap-modal"; m.hidden=true;
    m.innerHTML='<div class="swap-card"><div class="swap-head"><span>⚽ Log your match</span>'+
  ...
  function rdRender(){
    ...
    $("rdBody").innerHTML=
      ...
      '<div class="rd-grid">'+
        '<label class="rd-f"><span>Minutes played <em>optional</em></span><input id="rdScore" type="number" inputmode="numeric" placeholder="90" value="'+escAttr(String(score))+'"/></label>'+
        '<label class="rd-f"><span>Longest kick <em>yds</em></span><input id="rdDrive" type="number" inputmode="numeric" placeholder="55" value="'+escAttr(String(drive))+'"/></label>'+
      '</div>'+
      '<div class="rd-lbl">How did you play?</div>'+rdChips("driving", RD_DRIVING)+
      '<div class="rd-lbl">How did the body hold up?</div>'+rdChips("energy", RD_ENERGY)+
      '<button type="button" class="rd-save" id="rdSave">✓ Bank the match</button>'+
      '<div class="rd-foot">Your longest kick feeds the kick-distance trend on Home & Stats.</div>';
  }
  ...
  function rdSave(){
    ...
    if(score==null && drive==null && !rdSel.driving && !rdSel.energy){ ffToast("Add at least one thing about the match"); return; }
    // Match PR? Compare against every kick-distance entry BEFORE writing.
    ...
    if(drive!=null && drive>prevBest && prevBest>0){
      ...
      ffToast("🚀 Longest kick — "+Math.round(drive)+" yds. That’s the gym showing up.");
    } else {
      ffToast("Match banked ⚽");
    }
  ...
  function rdInsights(rounds){
    var out=[];
    // 1 · Minutes trend — first 3 vs last 3 matches with minutes logged (needs 5+ so they don't overlap much).
    // Minutes are higher-is-better, so the valence is flipped vs the old golf score (d = early - late).
    ...
    if(scored.length>=5){
      ...
      if(d>=2) out.push('🧊 <b>Minutes down '+d+' a match</b> — last 3 avg '+Math.round(late)+' vs '+Math.round(early)+' when you started. Keep earning it: sharp in training and strong to the whistle.');
      else if(d<=-2) out.push('📈 <b>Up '+(-d)+' minutes a match</b> — last 3 avg '+Math.round(late)+' vs '+Math.round(early)+'. The work is earning you time on the pitch.');
    }
    ...
    if(lifts.length){
      ...
      if(near.length>=2 && far.length>=2){
        ...
        if(nA-fA>=5) out.push('🏋️ <b>Kicks avg '+nA+' yds within 2 days of a lift</b> vs '+fA+' further out — keep matches close to gym days.');
        else if(fA-nA>=5) out.push('🛌 Kicks avg <b>'+fA+' yds with 3+ days after lifting</b> vs '+nA+' closer in — your legs like a little more space before kickoff.');
      }
    }
    ...
    if(fresh.length>=2 && loaded.length>=2){
      ...
      if(frA-loA>=5) out.push('🪫 <b>Recovery-week kicks avg '+frA+' yds</b> vs '+loA+' in loading weeks — that’s the deload doing its job. Plan big matches for fresh weeks.');
    }
    return out.slice(0,3);
  }

  // The Stats proof card: matches, best kick, and the stamina story.
  // Returns '' before the first match — the Stats "unlocks" strip advertises
  ...
  function courseCardHtml(){
    ...
    var h='';
    {
      ...
      if(bd>0) h+='<div class="pc-now">'+Math.round(bd)+'<span>yds best kick</span></div>';
      if(rated>0) h+='<div class="rd-stam">'+(strong===rated
        ? '💪 Finished <b>strong</b> in all '+rated+' rated match'+(rated===1?"":"es")+' — the endurance work is holding.'
        : '<b>'+strong+' of '+rated+'</b> matches finished strong. Fading late is a fuel + conditioning problem — both are in the plan.')+'</div>';
      ...
      // Receipts: what the training is doing for real matches, from the user's own data.
      ...
      if(ins.length){
        ...
        h+='<div class="rd-ins dim">🧾 Keep logging — at ~5 matches this card starts showing receipts: minutes trend, kicks near vs far from gym days, deload-week freshness.</div>';
      }
    }
    h+='<button type="button" class="fd-act" data-roundlog="1">⚽ Log a match ›</button>';
    return pfCard('course','⚽ On the pitch',
      (bd>0?'<span class="pf-num">'+Math.round(bd)+' yds</span>':'')+
      (rounds.length?'<span class="pc-delta neu">'+rounds.length+' match'+(rounds.length===1?"":"es")+'</span>':''), h);
  }
