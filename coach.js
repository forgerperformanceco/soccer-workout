...
(function () {
  ...
  function injectStyles() {
    var css = ''
      + '.ffc-wrap{position:fixed;inset:0;background:rgba(8, 15, 24, .5);z-index:80;display:flex;align-items:flex-end;'
      + 'justify-content:center;opacity:0;pointer-events:none;transition:opacity .2s;}'
      + '.ffc-wrap.open{opacity:1;pointer-events:auto;}'
      + '.ffc-sheet{background:#f6f8fa;width:100%;max-width:560px;height:88vh;max-height:88vh;border-radius:22px 22px 0 0;'
      + 'display:flex;flex-direction:column;overflow:hidden;transform:translateY(26px);transition:transform .22s;box-shadow:0 -8px 40px rgba(8, 18, 30, .35);}'
      ...
      + '.ffc-head{background:linear-gradient(160deg,#0f1824,#142f53);color:#fff;padding:6px 18px 14px;display:flex;'
      + 'align-items:center;justify-content:space-between;}'
      + '.ffc-head h3{margin:0;font-size:15px;display:flex;align-items:center;gap:8px;}'
      + '.ffc-head .ffc-ctx{margin:2px 0 0;font-size:11px;color:#bfd0e6;}'
      ...
      + '.ffc-msg.user{align-self:flex-end;max-width:85%;background:#113564;color:#fff;padding:10px 14px;border-radius:15px;border-bottom-right-radius:5px;white-space:pre-wrap;}'
      + '.ffc-msg.bot{align-self:stretch;color:#162130;padding:2px 2px 2px 13px;border-left:3px solid #cfd9e6;}'
      + '.ffc-msg.bot p{margin:0 0 9px;} .ffc-msg.bot p:last-child{margin-bottom:0;}'
      + '.ffc-msg.bot p.h{margin:13px 0 5px;font-size:14px;}'
      + '.ffc-msg.bot strong{color:#0f2c51;font-weight:700;}'
      + '.ffc-msg.bot ul,.ffc-msg.bot ol{margin:5px 0 10px;padding-left:20px;} .ffc-msg.bot li{margin:4px 0;}'
      + '.ffc-msg.bot code{background:#eaeef3;padding:1px 5px;border-radius:5px;font-size:13px;}'
      + '.ffc-msg.note{align-self:center;background:#eef1f6;color:#3f4f64;font-size:12.5px;text-align:center;max-width:96%;padding:11px 14px;border-radius:12px;line-height:1.5;}'
      + '.ffc-cta{background:#113564;color:#fff;border:0;border-radius:10px;padding:11px 16px;font:700 14px system-ui;cursor:pointer;margin-top:4px;}'
      + '.ffc-in{display:flex;gap:8px;padding:12px;border-top:1px solid #e2e6ec;background:#fff;}'
      + '.ffc-in textarea{flex:1;resize:none;border:1.5px solid #cdd4dd;border-radius:12px;padding:11px 13px;font:15px system-ui;max-height:96px;}'
      + '.ffc-send{background:#113564;color:#fff;border:0;border-radius:12px;padding:0 18px;font:700 14px system-ui;cursor:pointer;}'
      + '.ffc-send:disabled{opacity:.5;}'
      + '.ffc-dots span{display:inline-block;width:6px;height:6px;margin:0 1px;border-radius:50%;background:#9babbf;animation:ffcb 1s infinite;}'
